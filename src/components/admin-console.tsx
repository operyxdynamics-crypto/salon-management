...
export function AdminConsole({ data }: { data: AdminConsoleData }) {
  ...
  return <main className="min-h-screen bg-[#F7FAFC] text-[#1F2937]">
    <header className="bg-[#173279] text-white"><div className="mx-auto flex h-20 max-w-[1600px] items-center justify-between px-5 lg:px-8"><Link href="/admin" className="flex items-center gap-2.5"><BrandMark light /></Link><div className="flex items-center gap-3"><div className="hidden text-right sm:block"><p className="text-sm font-bold">{data.adminName}</p><p className="text-xs text-white/50">Super administrator</p></div><ShieldCheck size={20} /><form action="/api/v1/auth/logout" method="post"><button className="flex items-center gap-2 rounded-full bg-white/10 px-4 py-2.5 text-sm font-bold"><LogOut size={16} /> Log out</button></form></div></div></header>
    <div className="mx-auto max-w-[1600px] p-5 lg:p-8">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#1969A2]">Platform control center</p><h1 className="mt-2 font-serif text-4xl">Salon governance</h1><p className="mt-2 text-[#737174]">Onboarding, verification, branch publication, subscriptions, access, and system activity.</p></div><div className="flex gap-2"><Link href="/" className="flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-bold"><ExternalLink size={15} /> Marketplace</Link><button onClick={() => setModal(true)} className="primary"><Plus size={15} /> Create salon</button></div></div>
      ...
      ].map(([labelText, value, Icon]) => { const MetricIcon = Icon as typeof Building2; return <div key={String(labelText)} className="rounded-3xl bg-white p-5"><MetricIcon size={19} className="text-[#1969A2]" /><p className="mt-4 text-xs text-[#737174]">{String(labelText)}</p><strong className="mt-1 block text-2xl">{String(value)}</strong></div>; })}</div>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[390px_1fr]">
        <aside className="overflow-hidden rounded-3xl bg-white"><div className="border-b border-black/6 p-5"><div className="flex items-center gap-2 rounded-xl bg-[#F7FAFC] px-4 py-3"><Search size={16} /><input className="w-full bg-transparent text-sm outline-none" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search salons, GSTIN, activity" /></div><div className="mt-3 flex gap-2 overflow-auto">{["ALL", "DRAFT", "PENDING_REVIEW", "ACTIVE", "SUSPENDED", "ARCHIVED"].map((item) => <button key={item} onClick={() => setStatus(item)} className={`rounded-full px-3 py-2 text-xs font-bold ${status === item ? "bg-[#173279] text-white" : "bg-[#F7FAFC]"}`}>{label(item)}</button>)}</div></div><div className="max-h-[700px] overflow-y-auto p-2">{visible.map((tenant) => <button key={tenant.id} onClick={() => { setSelectedId(tenant.id); setTab("overview"); }} className={`w-full rounded-2xl p-4 text-left ${selected?.id === tenant.id ? "bg-[#E8FBFB]" : "hover:bg-[#F7FAFC]"}`}><div className="flex gap-3"><span className="grid size-10 place-items-center rounded-xl bg-white"><Store size={18} /></span><span className="min-w-0 flex-1"><span className="flex justify-between gap-2"><strong className="truncate text-sm">{tenant.name}</strong><Status value={tenant.status} /></span><span className="mt-1 block text-xs text-[#737174]">{tenant.branches.length} branch · {tenant.subscription?.planName ?? tenant.status}</span></span><ChevronRight size={15} /></div></button>)}{!visible.length && <p className="p-8 text-center text-sm text-[#737174]">No tenants match.</p>}</div></aside>
        <section>
          <div className="mb-4 flex gap-2">{(["overview", "documents", "categories", "activity"] as const).map((item) => <button key={item} onClick={() => setTab(item)} className={`rounded-full px-4 py-2 text-sm font-bold ${tab === item ? "bg-[#173279] text-white" : "bg-white"}`}>{label(item)}</button>)}</div>
...
function Overview({ tenant, plans, busy, mutate }: { tenant: Tenant; plans: Plan[]; busy: boolean; mutate: (path: string, body: unknown, success: string, method?: string) => Promise<boolean> }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-3xl bg-white p-6"><h3 className="font-serif text-2xl">Support notes</h3><div className="mt-4 space-y-2">{tenant.notes.map((item) => <div key={item.id} className="rounded-xl bg-[#F7FAFC] p-3 text-sm"><p>{item.note}</p><p className="mt-1 text-xs text-[#737174]">{item.author} · {formatDate(item.createdAt)}</p></div>)}</div><textarea className="field mt-4 min-h-20" value={note} onChange={(event) => setNote(event.target.value)} placeholder="Internal support note" /><button disabled={busy || note.trim().length < 2} onClick={() => void mutate(`/api/v1/admin/tenants/${tenant.id}/notes`, { note }, "Support note added.", "POST")} className="primary mt-3">Add note</button></section>
  </div>;
}
...
function Info({ label: text, value }: { label: string; value: string }) { return <div className="rounded-xl bg-[#F7FAFC] p-3"><p className="text-xs text-[#737174]">{text}</p><strong className="mt-1 block text-sm">{value}</strong></div>; }
