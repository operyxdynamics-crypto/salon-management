...
export function LoginPanel() {
  ...
  return (
    <div className="w-full max-w-md rounded-[2rem] bg-white p-7 shadow-xl sm:p-9">
      <Link href="/" className="mb-8 flex items-center gap-2 text-sm font-bold text-[#737174] lg:hidden"><ArrowLeft size={17} /> Back to {brandName}</Link>
      <div className="mb-8 lg:hidden"><BrandMark /></div>
      <p className="text-xs font-bold uppercase tracking-[.18em] text-[#1969A2]">Welcome back</p>
      ...
      <div className="mt-7 grid grid-cols-2 rounded-full bg-[#f2efea] p-1">
        <button onClick={() => { setMode("staff"); setError(""); }} className={`rounded-full py-2.5 text-sm font-bold ${mode === "staff" ? "bg-white shadow-sm" : "text-[#737174]"}`}>Salon team</button>
        <button onClick={() => { setMode("customer"); setError(""); }} className={`rounded-full py-2.5 text-sm font-bold ${mode === "customer" ? "bg-white shadow-sm" : "text-[#737174]"}`}>Customer</button>
      </div>
      {mode === "staff" ? (
        <form onSubmit={submitStaff} className="mt-7 space-y-4">
          ...
          {showDemoHints && (
            <p className="text-center text-xs leading-5 text-[#8b837b]">Salon: owner@operyx.demo / Aero@1406<br />Admin: admin@operyx.demo / Aero@1406</p>
          )}
        </form>
      ) : (
...
function Input({ label, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  return <label className="block"><span className="text-sm font-bold">{label}</span><input {...props} className="mt-2 w-full rounded-xl border border-black/10 px-4 py-3.5 text-sm outline-none transition focus:border-[#1969A2]" required /></label>;
}

function Submit({ loading, children }: { loading: boolean; children: React.ReactNode }) {
  return <button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-full bg-[#173279] py-3.5 text-sm font-bold text-white disabled:opacity-60">{loading && <LoaderCircle size={17} className="animate-spin" />}{children}</button>;
}
