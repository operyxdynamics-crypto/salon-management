...
type Data = {
  ...
  status: string;
  services: Array<{ id: string; name: string; category: string; durationMinutes: number; price: string | number }>;
...
export function OnboardingWorkspace() {
  ...
  async function addService(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/v1/onboarding/services", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: form.get("name"), category: form.get("category"), durationMinutes: Number(form.get("durationMinutes")), price: Number(form.get("price")), taxRate: 18 }) });
  ...
  if (!data) return <main className="grid min-h-screen place-items-center bg-[#f3f1ed]"><p>Loading onboarding...</p></main>;
  const branch = data.branches.find((item) => item.id === selectedBranchId) ?? data.branches[0];
  if (!branch) return <main className="grid min-h-screen place-items-center bg-[#f3f1ed]"><p>No branch is configured.</p></main>;
  ...
  return <main className="min-h-screen bg-[#f3f1ed] text-[#252320]">
    <header className="bg-[#203a36] text-white"><div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5"><Link href="/" className="flex items-center gap-2 font-serif text-2xl font-bold"><BrandMark light /></Link><div className="flex items-center gap-3"><span className="rounded-full bg-white/10 px-4 py-2 text-sm font-bold">{label(branch.publicationStatus)}</span><form action="/api/v1/auth/logout" method="post"><button className="flex items-center gap-2 text-sm font-bold"><LogOut size={16} /> Log out</button></form></div></div></header>
    <div className="mx-auto max-w-7xl p-5 lg:p-8">
      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        <div className="space-y-6">
          <section className="rounded-3xl bg-white p-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#9e5d55]">Salon onboarding</p><h1 className="mt-2 font-serif text-4xl">Prepare {data.name} for approval.</h1><p className="mt-3 text-[#746d66]">Each branch has its own profile, documents, checklist, and publication decision.</p><div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto]"><select className="field" value={branch.id} onChange={(event) => setSelectedBranchId(event.target.value)}>{data.branches.map((item) => <option key={item.id} value={item.id}>{item.name} · {label(item.publicationStatus)}</option>)}</select><span className="rounded-full bg-[#f1e7e2] px-4 py-3 text-center text-xs font-bold text-[#9e5d55]">{data.branches.length} branch{data.branches.length === 1 ? "" : "es"}</span></div>{notice && <p className="mt-4 rounded-xl bg-[#dff0e7] p-3 text-sm font-bold text-[#285543]">{notice}</p>}{error && <p className="mt-4 rounded-xl bg-[#f2ded8] p-3 text-sm font-bold text-[#995849]">{error}</p>}</section>
          ...
          <form onSubmit={saveProfile} className="rounded-3xl bg-white p-6">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2"><Field name="name" label="Trading name" defaultValue={data.name} /><Field name="legalName" label="Legal business name" defaultValue={data.legalName ?? ""} required={false} /><Field name="gstin" label="GSTIN" defaultValue={data.gstin ?? ""} required={false} hint="Example: 29ABCDE1234F1Z5" uppercase /><Field name="panNumber" label="PAN" defaultValue={data.panNumber ?? ""} required={false} hint="Example: ABCDE1234F" uppercase /><Field name="branchName" label="Branch name" defaultValue={branch.name} /><Field name="phone" label="Branch mobile" defaultValue={branch.phone ?? ""} required={false} hint="10 digits or +91 format" /><Field name="email" label="Branch email" type="email" defaultValue={branch.email ?? ""} required={false} /><Field name="postalCode" label="PIN code" defaultValue={branch.postalCode} required={false} inputMode="numeric" /><Field name="address" label="Address" defaultValue={branch.address} wide required={false} /><Field name="city" label="City" defaultValue={branch.city} /><Field name="state" label="State" defaultValue={branch.state} required={false} /><Field name="cancellationHours" label="Free cancellation hours" type="number" defaultValue={String(branch.policies?.cancellationHours ?? 4)} /><label className="sm:col-span-2 text-sm font-bold">Marketplace description<textarea className="field mt-2 min-h-28" name="profileDescription" defaultValue={branch.profileDescription ?? ""} /><span className="mt-1 block text-xs font-normal text-[#817970]">At least 20 characters before branch submission.</span></label></div>
            <h3 className="mt-7 font-bold">Operating hours</h3><div className="mt-3 grid gap-2">{hours.map((item) => <div key={item.dayOfWeek} className="grid grid-cols-[1fr_100px_100px_auto] items-center gap-2 rounded-xl bg-[#f6f3ee] p-3 text-sm"><strong>{dayNames[item.dayOfWeek]}</strong><input className="field !p-2" name={`open-${item.dayOfWeek}`} type="time" defaultValue={item.opensAt} /><input className="field !p-2" name={`close-${item.dayOfWeek}`} type="time" defaultValue={item.closesAt} /><label className="flex gap-1"><input name={`closed-${item.dayOfWeek}`} type="checkbox" defaultChecked={item.isClosed} /> Closed</label></div>)}</div>
            <button disabled={!editable || busy} className="primary mt-6 justify-center">{busy ? "Saving..." : "Save profile"}</button>
          </form>
          <section className="rounded-3xl bg-white p-6"><h2 className="font-serif text-2xl">Service catalogue</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{data.services.map((service) => <div key={service.id} className="rounded-2xl border border-black/8 p-4"><strong>{service.name}</strong><p className="mt-1 text-sm text-[#746d66]">{service.category} · {service.durationMinutes} min · {inr.format(Number(service.price))}</p></div>)}</div>{editable && <form onSubmit={addService} className="mt-5 grid gap-3 sm:grid-cols-4"><input className="field" name="name" placeholder="Service name" required /><input className="field" name="category" placeholder="Category" required /><input className="field" name="durationMinutes" type="number" placeholder="Minutes" required /><input className="field" name="price" type="number" placeholder="Price" required /><button className="primary justify-center sm:col-span-4"><Plus size={15} /> Add service</button></form>}</section>
          <section className="rounded-3xl bg-white p-6"><h2 className="font-serif text-2xl">Verification documents</h2><div className="mt-4 space-y-2">{data.verificationDocuments.map((document) => <div key={document.id} className="flex items-center justify-between rounded-xl border border-black/8 p-3 text-sm"><div><strong>{label(document.type)}</strong><p className="text-[#746d66]">{document.fileName}{document.reviewNote ? ` · ${document.reviewNote}` : ""}</p></div><Status value={document.status} /></div>)}</div>{editable && <form onSubmit={upload} className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><select className="field" name="type">{documentTypes.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select><input type="hidden" name="branchId" value={branch.id} /><input className="field" type="file" name="file" accept=".pdf,image/jpeg,image/png,image/webp" required /><button disabled={busy} className="primary justify-center"><FileUp size={15} /> Upload</button></form>}</section>
        </div>
        <aside className="h-fit rounded-3xl bg-white p-6 lg:sticky lg:top-6"><h2 className="font-serif text-2xl">Approval checklist</h2><div className="mt-5 space-y-4">{Object.entries(branch.checklist).map(([key, done]) => <div key={key} className="flex items-start gap-3 text-sm">{done ? <Check className="mt-0.5 shrink-0 text-[#2f6a55]" size={18} /> : <Circle className="mt-0.5 shrink-0 text-[#b8afa6]" size={18} />}<span><span className={done ? "font-bold" : "text-[#746d66]"}>{label(key)}</span>{!done && checklistDetails[key] && <span className="mt-1 block text-xs leading-5 text-[#9a6258]">{checklistDetails[key]}</span>}</span></div>)}</div><button disabled={!complete || !editable} onClick={submitBranch} className="primary mt-6 w-full justify-center"><Send size={15} /> Submit branch</button>{!complete && <p className="mt-3 rounded-xl bg-[#f6f3ee] p-3 text-xs leading-5 text-[#817970]">{submissionHelp(checklistDetails)}</p>}{branch.reviewsHistory[0]?.note && <div className="mt-5 rounded-xl bg-[#f2e8e2] p-4 text-sm"><strong>Latest review note</strong><p className="mt-2 text-[#746d66]">{branch.reviewsHistory[0].note}</p></div>}</aside>
      </div>
    </div>
  </main>;
}

function Field({ name, label: text, type = "text", defaultValue, wide, required = true, hint, uppercase, inputMode }: { name: string; label: string; type?: string; defaultValue: string; wide?: boolean; required?: boolean; hint?: string; uppercase?: boolean; inputMode?: "numeric" }) { return <label className={`${wide ? "sm:col-span-2" : ""} text-sm font-bold`}>{text}<input className={`field mt-2 ${uppercase ? "uppercase" : ""}`} required={required} name={name} type={type} defaultValue={defaultValue} inputMode={inputMode} />{hint && <span className="mt-1 block text-xs font-normal text-[#817970]">{hint}</span>}</label>; }
